...
;;; SSSP Geometry Tool - Integrated Main Entry Point
;;; ============================================================
;;; 
...
(define (make-morton-grid-graph width height)
  "Generate Morton-encoded grid graph with width x height nodes.
   Returns a function that computes neighbors for a given node-id."
  (lambda (node-id)
         ...
         (lambda (c)
           (let ((cx (car c)) (cy (cadr c)))
             (when (and (>= cx 0) (< cx width) 
                        (>= cy 0) (< cy height))
               ;; Return as (neighbor . weight) pairs for SSSP
               (set! neighbors (cons (cons (morton2-encode cx cy) 1.0) neighbors)))))
         candidates)
        neighbors))))

(define (make-hypercube-graph bits)
  "Generate hypercube graph with 2^bits nodes.
   Nodes are n-bit integers, edges connect nodes with Hamming distance 1."
  (lambda (node-id)
...
(define (manhattan-dist node-a node-b)
  "Calculate Manhattan distance between two Morton-encoded nodes"
...
(define (simple-bfs-sssp graph-fn start end)
  "Simple BFS shortest path for unweighted graphs.
   graph-fn: function (node-id) -> list of (neighbor . weight) pairs
