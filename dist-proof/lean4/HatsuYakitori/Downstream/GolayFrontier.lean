...
namespace HatsuYakitori.Downstream.GolayFrontier
...
theorem galoisHeightBound_pos : galoisHeightBound > 0 := by
  simp only [galoisHeightBound]
  norm_num
...
noncomputable def golayToGaloisHeight (c : Codeword) : ℝ :=
  (hammingWeight c.val : ℝ) / 24 * galoisHeightBound
...
theorem golayToGaloisHeight_le_bound (c : Codeword) :
    golayToGaloisHeight c ≤ galoisHeightBound := by
  simp only [golayToGaloisHeight]
  have hw : hammingWeight c.val ≤ 24 := hammingWeight_le_24 c
  have hK : galoisHeightBound > 0 := galoisHeightBound_pos
  ...
  calc (hammingWeight c.val : ℝ) / 24 * galoisHeightBound
      ≤ 1 * galoisHeightBound := by
        apply mul_le_mul_of_nonneg_right hdiv (le_of_lt hK)
    _ = galoisHeightBound := one_mul _
...
theorem frontierMode_consistent (c : Codeword) :
  ...
  · -- h1: τ < 12, h2: ¬((τ/24)*K < K/2) — contradiction
    ...
    calc (hammingWeight c.val : ℝ) / 24 * galoisHeightBound
        < (1/2) * galoisHeightBound := mul_lt_mul_of_pos_right hdiv hK
      _ = galoisHeightBound / 2 := by ring
  · -- h1: ¬(τ < 12), h2: (τ/24)*K < K/2 — contradiction
    exfalso
    push_neg at h1
    have hw : (hammingWeight c.val : ℝ) ≥ 12 := Nat.cast_le.mpr h1
    have hdiv : (hammingWeight c.val : ℝ) / 24 ≥ 1/2 := by linarith
    have hge : (hammingWeight c.val : ℝ) / 24 * galoisHeightBound
               ≥ galoisHeightBound / 2 := by
      have : (1/2 : ℝ) * galoisHeightBound = galoisHeightBound / 2 := by ring
      rw [← this]
