...
namespace HatsuYakitori.Upstream.CartanUtils
...
-/
noncomputable def machineEpsilon : ℝ := machineEpsilonReal

/-- Default tolerance for decomposition lookups.
    Sourced from `HatsuYakitori.MachineConstants`. -/
noncomputable def defaultTolerance : ℝ := defaultToleranceReal
