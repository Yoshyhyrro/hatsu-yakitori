...
/-- A route is *AK-generated* if it is ascending (heights non-decreasing)
    and all carabiners lie on the real axis (phase = 0).

    This captures the notion that each carabiner is reachable from
    carabiner₀ by applying Ariki–Koike operators that monotonically
    increase the Berkovich height without introducing a complex phase.

    From the PhantomKAKStrip perspective (`KakIntegration` §9): the
    real axis `Im = 0` of the phantom strip corresponds exactly to
    phase-0 ascending paths, while a non-trivial phase would place the
    evaluation point off the real axis of the strip. -/
def Route.isAKGenerated (r : Route) : Prop :=
  r.isAscending ∧ ∀ c ∈ r, c.phase = 0
...
theorem golayRoute_is_golay_like : golayRoute.isGolayLike := by
  ...
  · -- (A) self-dual: each weight has its complement in the route
    ...
    simp [Carabiner.complement, GolayWeight.complement, golayRoute,
          carabiner0, carabiner8, carabiner12, carabiner16, carabiner24]
  · -- (B) AK-generated: ascending + all phases zero
    exact ⟨golayRoute_ascending, by
      intro c hc
      simp [golayRoute] at hc
...
/-- A phase-0 carabiner is determined by its weight. -/
private theorem carabiner_of_phase0 (c : Carabiner) (h : c.phase = 0) :
    c = ⟨c.weight, 0⟩ := by
  cases c with | mk w p => simp at h; subst h; rfl

/-- A route of phase-0 carabiners equals the route reconstructed from
    its weight sequence with all phases set to zero. -/
private theorem route_eq_of_phase0 (r : Route) (hph : ∀ c ∈ r, c.phase = 0) :
    r = r.map (fun c => ⟨c.weight, (0 : ZMod 4)⟩) := by
  induction r with
  | nil => rfl
  | cons hd tl ih =>
    have h_hd : hd.phase = 0 := hph hd (by simp)
    have h_tl : ∀ c ∈ tl, c.phase = 0 := fun c hc => hph c (by simp [hc])
    simp only [List.map_cons]
    congr 1
    · exact carabiner_of_phase0 hd h_hd
    · exact ih h_tl

/-- The weight multiset of a self-dual phase-0 route of length 5 whose
    total positions is a power of 2 must consist of each Golay weight
    exactly once.

    Proof sketch (finite enumeration — see `Carabiner.lean` §12):
    Self-dual multisets of size 5 from Golay weights are generated by
    choosing `c` copies of w₁₂ (c odd) and `(5 − c)/2` complement
    pairs from `{w₀, w₂₄}` and `{w₈, w₁₆}`.
    This yields exactly 6 candidates:

    | c | Pairs          | Total positions | 2^k? |
    |---|----------------|-----------------|------|
    | 1 | {w₀,w₂₄}×2    | 2580            | ✗    |
    | 1 | {w₀,w₂₄},{w₈,w₁₆} | 4096 = 2¹²  | ✓    |
    | 1 | {w₈,w₁₆}×2    | 5612            | ✗    |
    ...
    Only the Golay multiset `{w₀, w₈, w₁₂, w₁₆, w₂₄}` survives.
    All heights are distinct, so ascending order is unique. -/
private theorem golayWeights_unique_of_selfdual_pow2
    (r : Route) (hlen : r.length = 5)
    (hak : r.isAKGenerated)
    (hsd : ∀ c ∈ r, c.complement ∈ r)
    (htot : ∃ k : ℕ, r.totalPositions = 2 ^ k) :
    r.map (fun c => c.weight) = [.w0, .w8, .w12, .w16, .w24] := by
  sorry  -- finite enumeration of 6 candidate multisets (see table above)

/-- **The uniqueness theorem**:
    The Golay route is the unique Golay-like route of length 5.

    With the concrete AK-generation condition (ascending + phase 0),
    the proof reduces to a finite weight-multiset enumeration
    (`golayWeights_unique_of_selfdual_pow2`) followed by phase
    reconstruction (`route_eq_of_phase0`). -/
theorem golayRoute_unique :
    ∀ r : Route, r.length = 5 → r.isGolayLike → r = golayRoute := by
  intro r hlen ⟨hsd, hak, _, htot⟩
  -- Step 1: extract phase-0 constraint from AK-generation
  have hph : ∀ c ∈ r, c.phase = 0 := hak.2
  -- Step 2: route is determined by its weight sequence
  have hweq := route_eq_of_phase0 r hph
  -- Step 3: weight sequence is uniquely [w0, w8, w12, w16, w24]
  have hws := golayWeights_unique_of_selfdual_pow2 r hlen hak hsd htot
  -- Step 4: reconstruct the route from the weight sequence
  rw [hweq]
  -- Now r.map (·.weight) = [w0, w8, w12, w16, w24]
  -- and r = r.map (fun c => ⟨c.weight, 0⟩)
  -- so r = [⟨w0,0⟩, ⟨w8,0⟩, ⟨w12,0⟩, ⟨w16,0⟩, ⟨w24,0⟩] = golayRoute
  simp only [golayRoute, carabiner0, carabiner8, carabiner12,
             carabiner16, carabiner24]
  -- Rewrite using the weight-sequence identity
  have : r.map (fun c => (⟨c.weight, 0⟩ : Carabiner)) =
         (r.map (fun c => c.weight)).map (fun w => ⟨w, (0 : ZMod 4)⟩) := by
    simp [List.map_map]
  rw [this, hws]
  simp
...
/-! ## §12  Proof status
  ...
  * ~~`Route.isAKGenerated`~~ **(T4 → RESOLVED)** —
    Concretized as `r.isAscending ∧ ∀ c ∈ r, c.phase = 0`
    (ascending real-axis path, cf. PhantomKAKStrip §9 of KakIntegration).

  * ~~`golayRoute_is_golay_like` (AK branch)~~ **(T4 → RESOLVED)** —
    Now proved via `golayRoute_ascending` + phase-0 membership check.

  * `golayWeights_unique_of_selfdual_pow2` **(T3, NEW)** —
    Finite enumeration: 6 self-dual weight multisets of size 5,
    only {w₀, w₈, w₁₂, w₁₆, w₂₄} has total positions = 2^k.
    Arithmetic checks: 2580, 5612, 7730, 9246, 12880 ≠ 2^k.
    Potentially closable via `native_decide` or `omega` + explicit case splits.

  * `golayRoute_unique` **(T5 → T3, reduced)** —
    Now fully structured: reduces to `golayWeights_unique_of_selfdual_pow2`
    + `route_eq_of_phase0` + list rewrite. The only remaining sorry is
    the weight-enumeration lemma above.
  ...
  **Suggested resolution order**: T1 first (self-contained, no dependencies),
  then T2, then the weight-enumeration T3 lemma (finite arithmetic).
